...
@forelse ($user->unreadNotifications as $notification)
    ...
    @foreach ($worksuitePlugins as $item)
        @if(View::exists(strtolower($item).'::notifications.client.'.\Illuminate\Support\Str::snake(class_basename($notification->type))))
            @include(strtolower($item).'::notifications.client.'.\Illuminate\Support\Str::snake(class_basename($notification->type)))
        @endif
    @endforeach
